  ...
  ensures  multiset(c[..]) == multiset(old(a[..])) + multiset(old(b[..]))
{
  ...
  while i < a.Length || j < b.Length
    ...
    invariant forall t :: 0 <= t < k-1 ==> c[t] <= c[t+1]
  ...
  ensures a[..] + b[..] == c[..]
{
  ...
  while i < a.Length
    // TO DO 
    invariant 0 <= i <= a.Length
    invariant forall t :: 0 <= t < i ==> c[t] == a[t]
  {
    c[i] := a[i];
    i := i + 1;
  }
  ...
  while j < b.Length
    // TO DO
    invariant 0 <= j <= b.Length
    invariant forall t :: 0 <= t < j ==> c[a.Length + t] == b[t]
    invariant forall t :: 0 <= t < i ==> c[t] == a[t]
  {
    c[a.Length + j] := b[j];
    j := j + 1;
  ...
  ensures d ==> exists i,j :: 0 <= i < j < |nums| && nums[i] == nums[j]
{
  ...
  for i:= 0 to |nums|
    // TO DO
    invariant 0 <= i <= |nums|
    invariant forall j :: 0 <= j < i ==> nums[j] in numSet
    invariant forall x :: x in numSet ==> exists j :: 0 <= j < i && nums[j] == x
  {
    if nums[i] in numSet {
      return true;
    }
...
method FindCommonElement(f: array<int>, g: array<int>, h: array<int>) returns (x: nat, y: nat, z: nat)
    ...
    ensures f[x] == g[y] == h[z]
  {
    x, y, z := 0, 0, 0;
    while true // FILL IN
      decreases (f.Length - x) + (g.Length - y) + (h.Length - z)
...
method LongestAscending(arr: array<int>) returns (i: nat, j: nat)
    ...
    ensures forall k, l :: 0 <= k <= l < arr.Length && AscendingInRange(arr, k, l) ==> l - k <= j - i
  {
    ...
    while j' < arr.Length - 1
      ...
      invariant 0 <= i' <= j' < arr.Length
      invariant AscendingInRange(arr, i, j)
      invariant AscendingInRange(arr, i', j')
      invariant i' == 0 || arr[i'-1] > arr[i']
      invariant forall k, l :: 0 <= k <= l < i' && AscendingInRange(arr, k, l) ==> l - k <= j - i
    {
      if arr[j'] <= arr[j' + 1] {
        j' := j' + 1;
      } else {
    ...
    ensures isPrime <==> (m > 1 && forall j : int :: 2 <= j < m ==> m % j != 0)
{
    ...
    while j >= 2
      decreases j
      invariant 1 <= j <= m
      invariant forall k : int :: j < k < m ==> m % k != 0
    {
      if m % j == 0 && j != m {
        return;
      } 
...
method TwoSum(arr: array<int>, target: int) returns (i: int, j: int)
  ...
  ensures i != j
  ensures 0 <= i < j < arr.Length
  ensures arr[i] + arr[j] == target
{
  i := 0;
  while i < arr.Length - 1
    decreases arr.Length - i
    invariant 0 <= i <= arr.Length - 1
    invariant forall k, l :: 0 <= k < i && k < l < arr.Length ==> arr[k] + arr[l] != target
  {
    j := i + 1;
    // Inner loop searches for a q such that arr[i]+arr[q]==target
    while j < arr.Length
      decreases arr.Length - j
      invariant 0 < j <= arr.Length
      invariant forall k :: i < k < j ==> arr[i] + arr[k] != target
    {
  ...
  ensures a.Length + b.Length == c.Length
{
  ...
  while j < b.Length
    ...
    invariant 0 <= j <= b.Length
    invariant forall t :: 0 <= t < i ==> c[t] == a[t]
    invariant forall t :: 0 <= t < j ==> c[t + a.Length] == b[t]
  {
    c[j + a.Length] := b[j];
    j := j + 1;
  }
...
function BinaryToNat(b: binary): nat {
  match b
  case Nil => 0
  case Cons(false, b') => 2 * BinaryToNat(b')
  case Cons(true, b') => 1 + 2 * BinaryToNat(b')
}
...
function NatToBinary(n: nat): (b:binary)
ensures WellFormedBinary(b)
{
  // Cons(if n % 2 == 0 then false else true, 
  //      if n/2 == 0 then Nil else NatToBinary(n / 2))
    Cons(n % 2 != 0, if n / 2 == 0 then Nil else NatToBinary(n / 2))
}
...
lemma {:induction false} NatToBinary_and_back(n: nat)
  ensures BinaryToNat(NatToBinary(n)) == n {
  if n == 0 {} else {NatToBinary_and_back(n / 2);}
}
...
// A binary is well-formed iff it is non-empty and either a single bit
// (allowed to be false for the representation of 0), or else the most
// significant bit is true. This enforces a unique, canonical
// representation (no multi-bit numbers that end with 0).
predicate WellFormedBinary(b: binary) {
  ...
  case Cons(_, b') =>
    match b'
    case Nil => true
    case Cons(node, Nil) => node
    case Cons(_, _) => WellFormedBinary(b')
}
...
//   assert !WellFormedBinary(Cons(false,Cons(true, Cons(false, Nil))));
// }

// Then you should be able to prove the remaining round-tripping law.
lemma {:induction false} BinaryToNat_and_back(b: binary)
